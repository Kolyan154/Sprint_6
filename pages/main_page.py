...
class MainPage(BasePage):
    ...
    @allure.step("Скролл к разделу с вопросами")
    ...
    @allure.step("Закрытие баннера cookie")
    def close_cookie_banner(self):
        ...
        if cookie_banner and cookie_banner.is_displayed():
            ...
            if cookie_button:
                ...
                self.wait_for_element_invisible(MainPageLocators.COOKIE_BANNER, timeout=3)
        else:
            # Если баннера нет, просто проскроллим немного вниз
            self.execute_script("window.scrollBy(0, 100);")

    @allure.step("Клик по кнопке заказа по локатору")
    def click_order_button_by_locator(self, button_locator):
        """Клик по кнопке заказа по указанному локатору"""
        self.scroll_to_element(button_locator)
        self.wait_for_element_clickable(button_locator)
